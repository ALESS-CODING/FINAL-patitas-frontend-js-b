...
window.addEventListener('load', function () {
    ...
    btnIngresar.addEventListener('click', function () {
        ...
        if (tipoDocumento.value === null || tipoDocumento.value.trim() === '' ||
            ...
            mostrarAlertaSuccess('Error: Debe completar correctamente sus credenciales');
            return;
        }
        ocultarAlerta();
        autenticar();
...
async function autenticar() {
    ...
    try {
        ...
        if (!response.ok) {
            mostrarAlertaSuccess('Error: Ocurrió un problema con la autenticación');
        ...
        if (result.codigo === '00') {
            ...
            mostrarAlertaSuccess(result.mensaje);
        }
